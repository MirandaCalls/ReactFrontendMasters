...
const ColorPaletteModal = ({navigation}) => {
  ...
  const hasEnoughColors = Object.keys(selectedColors).length >= 3;
  const hasName = paletteName.length > 0;
  if (hasEnoughColors && hasName) {
    isSubmitDisabled = false;
  }

  const submitStyles = [styles.submitButton];
  if (isSubmitDisabled) {
    submitStyles.push(styles.disabled);
  }

  const onColorToggled = useCallback(({ enabled, colorName, colorHex }) => {
    setSelectedColors((current) => {
      const colorsCopy = { ...current };
      if (enabled) {
        colorsCopy[colorName] = colorHex;
      } else {
        delete colorsCopy[colorName];
      }
  ...
  const onPaletteSubmitted = useCallback(() => {
	const newPalette = {
		id: (new Date()).getTime(),
	...
	for (let colorName in selectedColors) {
		newPalette.colors.push({
			colorName: colorName,
			hexCode: selectedColors[colorName]
		});
	}
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.formLabel}>Color palette name</Text>
      <TextInput
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ColorToggle
            ...
            onTogglePress={onColorToggled}
          />
        )}
        style={styles.colorsPicker}
      />
      <TouchableOpacity style={submitStyles} disabled={isSubmitDisabled} onPress={onPaletteSubmitted}>
        <Text style={styles.submitButtonText}>Add palette</Text>
      </TouchableOpacity>
    </View>
  );
};
